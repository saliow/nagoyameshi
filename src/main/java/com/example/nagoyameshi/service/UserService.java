...
public class UserService {
	...
	public User createUser(SignupForm signupForm) {
        ...
        if (!signupForm.getBirthday().isEmpty()) {
        	user.setBirthday(LocalDate.parse(signupForm.getBirthday(), DateTimeFormatter.ofPattern("yyyyMMdd")));//birthdayフィールドに値をセットする前に、LocalDateクラスのparse()メソッドを使って String型からLocalDate型に変換しておきましょう。
        } else {
        ...
        user.setPassword(passwordEncoder.encode(signupForm.getPassword()));//Userエンティティのpasswordフィールドに値をセットする前に、PasswordEncoderインターフェースのencode()メソッドを使ってハッシュ化しておきましょう。
        user.setRole(role);
 ...
 // パスワードとパスワード（確認用）の入力値が一致するかどうかをチェックする
    ...
    //指定されたキーワードを氏名又はフリガナに含むユーザーを、ページングされた状態で取得する。
